...
## Task 3 — bootstrapping the state bucket
...
Unlike a resource, a `check` never halts a run on failure — it surfaces a warning, which makes it useful for catching drift or misconfiguration without blocking deployments.
...
| Backend | Notes |
|---|---|
| **S3** | Cheapest, most common for AWS-only setups. Native locking since Terraform 1.11 means no DynamoDB table is needed anymore. |
